...
#include "../src/ReadTree.cc"
#include "../src/SetStyle.C"
#include "../src/StackPlotLDP.C"
#include "../src/ComputeBinQCD.C"
#include <stdio.h>
using namespace std;
void ZinvEstimateQCDLDP(){//main programme
     //Set Canvas Style
     ...
     tZinv->Add(InputFilePathV3+"tree_LDP/tree_ZJetsToNuNu_HT*.root");
     ReadTree Zinv(tZinv);


     //reading the GJets MC
     TChain* tGJets = new TChain("tree");
     tGJets->Add(InputFilePathV3+"tree_GJetLDP_CleanVars/tree_GJets_HT*.root");
     ...
     tQCD->Add(InputFilePathV3+"tree_GJetLDP_CleanVars/tree_QCD_HT*.root");
     ReadTree QCD(tQCD);

     //reading Single photon Data
     TChain* tData = new TChain("tree");
     tData->Add(InputFilePathV3+"tree_GJetLDP_CleanVars/tree_SinglePhoton_*.root");
     ReadTree Data(tData);
   ...
   //////////////////////////////////////////////Calculating Zgamma Ratio start
    ...
    /////////////////////////////////////////////////////Dealing with Data barrel
     ...
     TH1F *hECfrag=new TH1F("hECfrag","hECfrag",nBinAN,0.5,nBinANmax);    
                         //uncorel      //corel
     double ebpErr=sqrt((0.012*0.012)+(0.10*0.10));
     double ecpErr=sqrt((0.021*0.021)+(0.03*0.03));
    ...
    // double ecpErrCorel=0.046;
     ...
     for(int i=1;i<(nBinAN+1);i++){//set purity stuff
          hEBPurity->SetBinContent(i,0.98);
          hEBPurity->SetBinError(i,ebpErr);
          hECPurity->SetBinContent(i,0.96);
...
//printf("%4.3f",ZinvMCYield);
   ...
   //printf("bin %i :%i | %4.3f(%4.3f,+%4.3f-%4.3f) | %4.3f(%4.3f)  | %4.2f(+%4.3f-%4.3f) ",ibin,Nobserved,ZgR,ZgRuncorel_Rel_Err,ZgRcorel_Rel_Errup,ZgRcorel_Rel_Errlow,totalPurity,totalPurity_Rel_Err,YieldTotal,YieldTotal_Rel_ErrUp,YieldTotal_Rel_ErrLow);
    ...
    TMg_1D[0]->Draw("AP");
    TMg_1D[0]->GetXaxis()->SetLimits(0.5,55.5); 
